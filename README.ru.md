...
Глубоко переработанная версия [Steam Achievement Manager](https://github.com/gibbed/SteamAchievementManager) от gibbed. Добавлена темная тема, интеграция Steam Web API, режимы простоя, защита VAC, панель профиля, параллельная загрузка, локализация и множество улучшений. Полная совместимость с оригинальным Steam API.
...
- [Отличия от оригинала](#отличия-от-оригинала)
  ...
  - [Переработка интерфейса](#переработка-интерфейса)
...
Темная тема оформления для всех окон приложения:
...
Применено ко всем окнам:
- Кастомная отрисовка OwnerDraw для элементов ListView
- Темный TabControl с акцентной линией на активной вкладке
- Темные панели инструментов, строки состояния, контекстные меню и диалоги
...
| **Защищенные достижения** | Темно-красный фон (`#281919`) для неизменяемых достижений |
...
- Кастомные темные чекбоксы (отмечен = бирюзовая заливка с белой галочкой)
- Защищенные достижения с темно-красным фоном, заблокированы для изменения
...
SAM.API/                           -- Библиотека Steam API (БЕЗ ИЗМЕНЕНИЙ)
  ...
  Wrappers/                        -- Обертки интерфейсов (SteamUserStats, SteamApps и др.)
...
SAM.Picker/                        -- Главное приложение (браузер библиотеки игр)
  ...
  ProfilePanel.cs                  -- [НОВОЕ] Панель профиля Steam
  PlaytimeReader.cs                -- [НОВОЕ] Парсер localconfig.vdf для данных о времени
  SteamWebApi.cs                   -- [НОВОЕ] Клиент Steam Web API (достижения, профиль, значки)
  AppSettings.cs                   -- [НОВОЕ] Хранение настроек (API ключ)
  ActiveGamesForm.cs               -- [НОВОЕ] Менеджер активных idle-игр
  IdleSettingsDialog.cs            -- [НОВОЕ] Настройка режимов idle
  SettingsDialog.cs                -- [НОВОЕ] Настройки языка, отображения и API ключа
  Localization.cs                  -- [НОВОЕ] Локализация EN/RU
  DarkTheme.cs                     -- [НОВОЕ] Темная тема с кастомными рендерами
...
SAM.Game/                          -- Редактор достижений и статистик
  ...
  DarkTheme.cs                     -- [НОВОЕ] Темная тема для редактора
